...
function elex_cm_plugin_admin_notices() {
    if (!get_option("elex_first_installation_msg")) {
        if ( in_array( 'elex-catmode-rolebased-price/elex-catmode-rolebased-price.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            echo "<div class='updated'><strong>ELEX WooCommerce Catalog Mode, Wholesale & Role Based Pricing</strong> is activated. Go to <a href=".admin_url( 'admin.php?page=wc-settings&tab=elex_catalog_mode' ).">Settings</a> to configure.</div>";
        }
