...
class Elex_CM_Pricing_Discount_Settings extends WC_Settings_Page {
    ...
    function elex_cm_include_js() {
            wp_enqueue_script('jquery');
            wp_enqueue_script('elex-catalog-mode', ELEX_CATALOG_MODE_MAIN_URL_PATH . 'includes/elex-html-catalog-adjustment.js');
    }
	...
	public function output_sections() {
		...
		foreach ( $sections as $id => $label ) {
			echo '<li><a href="' . admin_url( 'admin.php?page=wc-settings&tab=elex_catalog_mode&section=' . sanitize_title( $id ) ) . '" class="' . ( $current_section == $id ? 'current' : '' ) . '">' . $label . '</a> ' . ( end( $array_keys ) == $id ? '' : '|' ) . ' </li>';
		}
    ...
    public function elex_cm_output_settings() {
        ...
        if($current_section == 'to-go-premium'){
              wp_enqueue_style('eh-catalog-bootstrap', ELEX_CATALOG_MODE_MAIN_URL_PATH . 'resources/css/bootstrap.css');
    ...
    function elex_cm_catalog_default_check_for_hide_addtocart ($post_id, $meta_key_shop, $meta_key_product) {
        $default_check = FALSE;
        if (isset($_POST['meta'])) {
