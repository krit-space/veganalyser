...
class MP_Product_List{
				...
				if ( $this->get_total_stock() > get_option( 'woocommerce_notify_no_stock_amount' ) ) {
					...
					switch ( $format_option ) {
						...
						case 'low_amount' :
							...
							$format = ( $this->get_total_stock() <= $low_amount ) ? __( 'Only ', 'marketplace' ) . '%s' . __( ' left in stock', 'marketplace' ) : __( 'In stock', 'marketplace' );
						break;
						default :
							$format = __( 'In stock', 'marketplace' );
						break;
	...
	public function get_rating_html( $rating = null ) {
		...
		if ( $rating > 0 ) {
			...
			$rating_html  = '<div class="star-rating" title="' . sprintf( __( 'Rated ', 'marketplace' ) . '%s' . __( ' out of 5', 'marketplace' ), $rating ) . '">';
