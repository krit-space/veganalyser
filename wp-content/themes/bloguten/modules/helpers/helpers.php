...
if( !function_exists( 'bloguten_enqueue' ) ):
/**
...
* @return void
*/
function bloguten_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'bloguten_scripts' , $scripts );
...
if( ! function_exists( 'bloguten_get_related_posts' ) ):
/**
* Get related posts
*
* @since Bloguten 1.0.0
...
* 
*/
function bloguten_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function bloguten_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'bloguten_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'bloguten_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function bloguten_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'bloguten' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'bloguten_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function bloguten_get_youtube_id( $url ) {
...
if( ! function_exists( 'bloguten_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Bloguten 1.0.0
* @uses bloguten_sort_category()
* @return array
*/
function bloguten_get_the_category( $id = false ){
    $failed = true;
    ...
    if ( class_exists( 'WPSEO_Primary_Term' ) ){
        ...
        $bloguten_cat[0] = get_term( $wpseo_primary_term );

        if ( !is_wp_error( $bloguten_cat[0] ) ) { 
           $failed = false;
        }
    }
    ...
    if( $failed ){
      ...
      $bloguten_cat = get_the_category( $id );
      usort( $bloguten_cat, 'bloguten_sort_category' );  
    }
...
if( ! function_exists( 'bloguten_sort_category' ) ):
/**
* Helper function for bloguten_get_the_category()
*
* @since Bloguten 1.0.0
*/
function bloguten_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'bloguten_explode_string_to_int' ) ):
/**
...
*/
function bloguten_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'bloguten_get_menu' ) ):
/**
* Returns wp nav
*
* @since Bloguten 1.0.0
...
* @return String
*/
function bloguten_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'bloguten_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'bloguten_'.$location.'_menu', $args ) );
        break;

        case 'secondary':
            $args[ 'fallback_cb' ] = 'bloguten_secondary_menu_fb';
        ...
        case 'footer':
            $args[ 'fallback_cb' ] = 'bloguten_footer_menu_fb';
...
if( !function_exists( 'bloguten_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Bloguten 1.0.0
*/
function bloguten_hex2rgba($color, $opacity = false) {
