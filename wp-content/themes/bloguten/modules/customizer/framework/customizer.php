...
if( ! class_exists( 'Bloguten_Customizer' ) ):

	class Bloguten_Customizer{
		...
		* 
		* @since  Bloguten 1.0.0
		* @uses   filter "Bloguten_Customizer_defaults"
		* @access public
		* @return array
		*/
	    public function get_defaults(){

	    	return apply_filters( 'Bloguten_Customizer_Defaults', array() );
	    }
		...
		* @since  Bloguten 1.0.0
		* @uses   get_defaults()
		* @uses   filter "Bloguten_Customizer_Fields"
		* @access public
		* @param  string $id
		* @return string | false
		*/
		public function get( $id ){
			...
			$fields   = apply_filters( 'Bloguten_Customizer_Fields', array() );
		...
		*/
		public function register( $wp_customize ){
			...
			do_action( 'bloguten_customize_register', $wp_customize );

			$this->customize = $wp_customize;

			$this->fields   = apply_filters( 'Bloguten_Customizer_Fields', array() );
			$this->panels   = apply_filters( 'Bloguten_Customizer_Panels', $this->panels );
			$this->sections = apply_filters( 'Bloguten_Customizer_Sections', $this->sections );

			$this->defaults = apply_filters( 'Bloguten_Customizer_Defaults', $this->defaults );
			...
			foreach( $this->fields as $key => $field ){
				...
				switch ( $field[ 'type' ] ) {
					...
					case 'number':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'bloguten_sanitize_number';
						}
					...
					case 'checkbox':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'bloguten_sanitize_checkbox';
						}
					...
					case 'radio':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'bloguten_sanitize_choice';
						}
					...
					case 'dropdown-pages':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'bloguten_sanitize_dropdown_pages';
					}
					break;
					
					case 'dropdown-posts':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'bloguten_sanitize_dropdown_posts';
					}
					break;
					...
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'bloguten_sanitize_dropdown_categories';
					}
		...
		*/
		protected function add_panels(){
			...
			foreach ( $this->panels as $id => $panel ) {
				...
				$args = array(
				    'title' => empty( $panel[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'bloguten' ) : $panel[ 'title' ],
				);
		...
		*/
		protected function add_sections(){
			...
			foreach ( $this->sections as $id => $section ){
				...
				$args = array(
					'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'bloguten' ) : $section[ 'title' ],
				);
		...
		protected function add_control( $field ){
			...
			switch( $field[ 'type' ] ){
				...
				case 'radio-image':
					...
					$this->customize->add_control( new Bloguten_Customize_Control_Radio_Image(
						$this->customize,
						$control_id,
						$args
					));
				...
				case 'dropdown-categories':
					...
					$this->customize->add_control( new Bloguten_Customize_Category_Control( 
						$this->customize, 
						$control_id, 
						$args 
					) );
				...
				case 'dropdown-posts':
					...
					$this->customize->add_control( new Bloguten_Customize_Post_Control( 
						$this->customize, 
						$control_id, 
						$args 
					) );
...
* 
* @since  Bloguten 1.0.0
* @uses   Bloguten_Customizer::get_instance()->get( $id )
* @return string | bool
*/
if( !function_exists( 'bloguten_get_option' ) ){

	function bloguten_get_option( $id ){

		return Bloguten_Customizer::get_instance()->get( $id );
	}

}

/**
* Retrives default value of Bloguten_Customizer.
* 
* @since  Bloguten 1.0.0
* @uses   Bloguten_Customizer::get_instance()->get_defaults()
* @return object
*/
if( !function_exists( 'Bloguten_Customizer_get_defaults' ) ){

	function Bloguten_Customizer_get_defaults(){

		return Bloguten_Customizer::get_instance()->get_defaults();
	}
}

add_action( 'customize_register', array ( Bloguten_Customizer::get_instance(), 'register' ) );
