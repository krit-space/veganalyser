...
function primaryHeader(){
	...
	onScroll = function(){
		...
		if( BLOGUTEN.is_admin_bar_showing && width >= 782 ){
			scroll = scroll+32;
		}
 	 	...
 	 	if( height ){
 	 		if( height >= scroll ){
 	 			...
 	 			removeClass();
 	 		}else if( BLOGUTEN.is_admin_bar_showing && width >= 782 ){
 	 			$( '#fixed-header' ).css({
 	 				top : 32
 	 			});
 	 			addClass();
 	 		}else if( BLOGUTEN.is_admin_bar_showing && width >= 601 && width <= 781 ){
 	 			$( '#fixed-header' ).css({
 	 				top : 46
 	 			});
 	 			addClass();
...
function homeSlider(){
	...
	jQuery(".home-slider").owlCarousel({
    	...
    	rtl: ( BLOGUTEN.is_rtl == '1' ) ? true : false,
    	responsive:{
    	    768:{
    	        items: 1,
    	        nav: true,
...
jQuery( document ).ready( function(){
	...
	*/
	new scrollToTop({
		markup   : '<a href="#page" class="scroll-to '+ ( BLOGUTEN.enable_scroll_top == 0 ? "d-none" : "" )+'" id="go-top"><span class="kfi kfi-arrow-up"></span></a>',
		selector : '#go-top'
	}).init();
	console.log(BLOGUTEN.enable_scroll_top + '' + 'test');
    ...
    */
    $( '#masthead #s' ).attr( 'placeholder', BLOGUTEN.search_placeholder );
    $( '#searchform #s' ).attr( 'placeholder', BLOGUTEN.search_default_placeholder );
});
